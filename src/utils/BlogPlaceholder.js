import React from "react";

const Placeholder = () => {
  return (
    <section className="text-gray-500 body-font">
      <div className="container px-5 py-16 mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          <div className="shadow-md">
            <div className="h-full border-2 border-gray-200 rounded-lg overflow-hidden">
              <div className="h-48 bg-gray-400 w-full object-cover object-center"></div>
              <div className="p-6">
                <div class="flex items-center flex-wrap mb-4">
                  <span class="bg-indigo-300 h-4 animate-pulse mt-2 w-32 inline-flex items-center md:mb-2 lg:mb-0"></span>
                  <span class="bg-indigo-300 w-16 mt-2 h-4 animate-pulse mr-3 px-2 inline-flex items-center ml-auto leading-none text-sm pr-5 py-1"></span>
                </div>
                <div className="w-2/3 mb-4 h-6 animate-pulse bg-gray-500"></div>
                <p className="leading-relaxed mb-3 w-full h-3 animate-pulse bg-gray-400"></p>
                <p className="leading-relaxed mb-3 w-full h-3 animate-pulse bg-gray-400"></p>
                <p className="leading-relaxed mb-3 w-2/3 h-3 animate-pulse bg-gray-400"></p>
                <p className="leading-relaxed mb-3 w-1/2 h-3 animate-pulse bg-gray-400"></p>
                <div className="flex items-center flex-wrap ">
                  <span className="bg-indigo-300 h-4 animate-pulse mt-2 w-32 inline-flex items-center md:mb-2 lg:mb-0"></span>
                </div>
              </div>
            </div>
          </div>
          <div className="shadow-md">
            <div className="h-full border-2 border-gray-200 rounded-lg overflow-hidden">
              <div className="h-48 bg-gray-400 w-full object-cover object-center"></div>
              <div className="p-6">
                <div class="flex items-center flex-wrap mb-4">
                  <span class="bg-indigo-300 h-4 animate-pulse mt-2 w-32 inline-flex items-center md:mb-2 lg:mb-0"></span>
                  <span class="bg-indigo-300 w-16 mt-2 h-4 animate-pulse mr-3 px-2 inline-flex items-center ml-auto leading-none text-sm pr-5 py-1"></span>
                </div>
                <div className="w-2/3 mb-4 h-6 animate-pulse bg-gray-500"></div>
                <p className="leading-relaxed mb-3 w-full h-3 animate-pulse bg-gray-400"></p>
                <p className="leading-relaxed mb-3 w-full h-3 animate-pulse bg-gray-400"></p>
                <p className="leading-relaxed mb-3 w-2/3 h-3 animate-pulse bg-gray-400"></p>
                <p className="leading-relaxed mb-3 w-1/2 h-3 animate-pulse bg-gray-400"></p>
                <div className="flex items-center flex-wrap ">
                  <span className="bg-indigo-300 h-4 animate-pulse mt-2 w-32 inline-flex items-center md:mb-2 lg:mb-0"></span>
                </div>
              </div>
            </div>
          </div>
          <div className="shadow-md">
            <div className="h-full border-2 border-gray-200 rounded-lg overflow-hidden">
              <div className="h-48 bg-gray-400 w-full object-cover object-center"></div>
              <div className="p-6">
                <div class="flex items-center flex-wrap mb-4">
                  <span class="bg-indigo-300 h-4 animate-pulse mt-2 w-32 inline-flex items-center md:mb-2 lg:mb-0"></span>
                  <span class="bg-indigo-300 w-16 mt-2 h-4 animate-pulse mr-3 px-2 inline-flex items-center ml-auto leading-none text-sm pr-5 py-1"></span>
                </div>
                <div className="w-2/3 mb-4 h-6 animate-pulse bg-gray-500"></div>
                <p className="leading-relaxed mb-3 w-full h-3 animate-pulse bg-gray-400"></p>
                <p className="leading-relaxed mb-3 w-full h-3 animate-pulse bg-gray-400"></p>
                <p className="leading-relaxed mb-3 w-2/3 h-3 animate-pulse bg-gray-400"></p>
                <p className="leading-relaxed mb-3 w-1/2 h-3 animate-pulse bg-gray-400"></p>
                <div className="flex items-center flex-wrap ">
                  <span className="bg-indigo-300 h-4 animate-pulse mt-2 w-32 inline-flex items-center md:mb-2 lg:mb-0"></span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Placeholder;
